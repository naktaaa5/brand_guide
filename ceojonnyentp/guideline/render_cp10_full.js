...
const groups=[
 ...
 {id:'appendix',no:'부록',label:'실물집·검수 기준',parts:[15,16],desc:'전이 장치의 나머지 실물과 표현 검수 기준'}
];
...
const navHtml=groups.map(g=>{const ext=g.id==='appendix';
 ...
 const subs=nav.filter(x=>x[2]===g.id).map((x,i)=>`<a href="${ext?APPENDIX_PAGE+'#'+x[0]:'#'+x[0]}"${ext?extAttr:''}><span>${String(i+1).padStart(2,'0')}</span><b>${esc(x[1])}</b></a>`).join('');
